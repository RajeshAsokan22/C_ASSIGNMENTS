...
int main()
{
    int num,t1,t2,next_term; /* Declaring the variables */
    ...
    else if (num > 0)  /* Condition to check whether entered no is positive or not */
    {
	printf("%d, %d",t1,t2);
	next_term = t1 + t2; /* The value of next_term is addition of two previous numbers */
	while (next_term <= num)  /* The while loop will iterate up to the next_term is lesser or equal to "num" */ 
	{
    ...
    {
	printf("Invalid input"); /* If entered no is not positive, it will print "Invalid input" */
    }
